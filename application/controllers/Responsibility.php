...
class Responsibility extends Operator_Controller
{
    ...
    public function add($level)
    {
        ...
        if (!$input->draft_id || !$input->user_id) {
        ...
        $count_staffs = count($this->responsibility->join('user')->where('draft_id', $input->draft_id)->where('level', $level)->get_all());

        if ($count_staffs >= 2) {
            return $this->send_json_output(false, 'Jumlah staff terpilih sudah maksimal', 422);
        }
    ...
    public function unique_responsibility_match()
    {
        ...
        !$responsibility_id || $this->responsibility->where_not('responsibility_id', $responsibility_id);
        $responsibility = $this->responsibility->get();

        if ($responsibility) {
            $this->form_validation->set_message('unique_responsibility_match', $this->lang->line('toast_data_duplicate'));
            return false;
        }
        return true;
    }
