...
class Document extends Admin_Controller {
    ...
    public function search($page = null) {
        ...
        $documents = $this->document->like('document_name', $keywords)->orLike('document_year', $keywords)->paginate($page)->getAll();
        $tot = $this->document->like('document_name', $keywords)->orLike('document_year', $keywords)->getAll();
        $total = count($tot);
        $pagination = $this->document->makePagination(site_url('document/search/'), 3, $total);
        if (!$documents) {
