...
// -- add --        
        public function add()
	{
        ...
        if ($this->draft_reviewer->insert($input)) {
            ...
            $r_user_notif = $this->db->query("select r.user_id, firebase_token from reviewer r join user u on r.user_id=u.user_id where reviewer_id='".$input->reviewer_id."'")->row();
            $user_id_notif = $r_user_notif->user_id;
            $judul_notif = 'review draft baru';//
            $isi_notif = 'review draft baru '.$r_draft->draft_title;
            ...
            //firebase
            if($r_user_notif->firebase_token != ''){
                $firebase_token = $r_user_notif->firebase_token;
