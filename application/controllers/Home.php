...
class Home extends Operator_Controller
{
    ...
    public function index($page = null)
    {
        ...
        if ($ceklevel == 'superadmin' or $ceklevel == 'admin_penerbitan') {
            ...
            $drafts        = $this->home->join3('draft_reviewer', 'draft', 'draft')->join3('reviewer', 'draft_reviewer', 'reviewer')->join3('user', 'reviewer', 'user')->where('user.username', $cekusername)->get_all('draft');
            $drafts_newest = $this->home->join3('draft_reviewer', 'draft', 'draft')->join3('reviewer', 'draft_reviewer', 'reviewer')->join3('user', 'reviewer', 'user')->where('user.username', $cekusername)->limit(5)->order_by('entry_date', 'desc')->get_all('draft');
            ...
            $count['draft_edit']      = $this->home->join3('draft_author', 'draft', 'draft')->join3('author', 'draft_author', 'author')->join3('user', 'author', 'user')->where('user.username', $cekusername)->where('is_review', 'y')->where('is_edit', 'n')->where_not('draft_status', '99')->count('draft');
            $count['draft_layout']    = $this->home->join3('draft_author', 'draft', 'draft')->join3('author', 'draft_author', 'author')->join3('user', 'author', 'user')->where('user.username', $cekusername)->where('is_edit', 'y')->where('is_layout', 'n')->where_not('draft_status', '99')->count('draft');
            $count['draft_proofread'] = $this->home->join3('draft_author', 'draft', 'draft')->join3('author', 'draft_author', 'author')->join3('user', 'author', 'user')->where('user.username', $cekusername)->where('is_layout', 'y')->where('is_proofread', 'n')->where_not('draft_status', '99')->count('draft');
            $count['draft_approved']  = $this->home->join3('draft_author', 'draft', 'draft')->join3('author', 'draft_author', 'author')->join3('user', 'author', 'user')->where_not('draft_status', '99')->where_not('draft_status', '2')->where('user.username', $cekusername)->count('draft');
            ...
            $count['draft_sudah']    = $this->home->join3('responsibility', 'draft', 'draft')->join3('user', 'responsibility', 'user')->where_not('edit_notes', '')->where('user.username', $cekusername)->count('draft');
            $count['draft_belum']    = $this->home->join3('responsibility', 'draft', 'draft')->join3('user', 'responsibility', 'user')->where('edit_notes', '')->where_not('draft_status', 99)->where('user.username', $cekusername)->count('draft');
            ...
            $count['draft_total']    = $this->home->join3('responsibility', 'draft', 'draft')->join3('user', 'responsibility', 'user')->where('user.username', $cekusername)->count('draft');
            $count['draft_desk']     = $this->home->where('draft_status', 0)->count('draft');
            $count['draft_sudah']    = $this->home->join3('responsibility', 'draft', 'draft')->join3('user', 'responsibility', 'user')->where_not('layout_notes', '')->where('user.username', $cekusername)->count('draft');
            $count['draft_belum']    = $this->home->join3('responsibility', 'draft', 'draft')->join3('user', 'responsibility', 'user')->where('layout_notes', '')->where_not('draft_status', 99)->where('user.username', $cekusername)->count('draft');
