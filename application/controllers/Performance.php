...
class Performance extends Admin_Controller {
	...
	public function __construct()
    {
        ...
        $this->pages = 'performance';
				$performance_editor = $this->performance->select(['draft.draft_id','draft_title','username','category_name','edit_start_date','edit_deadline','edit_end_date','is_edit','performance_status'])->join3('category','draft','category')->join3('responsibility','draft','draft')->join3('user','responsibility','user')->where('level', 'editor')->where('responsibility.user_id !=', null)->orderBy('username')->getAll('draft');
  ...
  public function index()
    {
      ...
      $performance_editor = $this->performance->select(['draft.draft_id','draft_title','username','category_name','edit_start_date','edit_deadline','edit_end_date','is_edit','performance_status'])->join3('category','draft','category')->join3('responsibility','draft','draft')->join3('user','responsibility','user')->where('level', 'editor')->where('responsibility.user_id !=', null)->where('performance_status', NULL)->orWhere('performance_status', 1)->orWhere('performance_status', 2)->orWhere('performance_status', 5)->orderBy('username')->getAll('draft');
  ...
  public function performa_layouter()
    {
      $performance_layouter = $this->performance->select(['draft.draft_id','draft_title','username','category_name','layout_start_date','layout_deadline','layout_end_date','is_layout','performance_status'])->join3('category','draft','category')->join3('responsibility','draft','draft')->join3('user','responsibility','user')->where('level', 'layouter')->where('responsibility.user_id !=', null)->where('performance_status', NULL)->orWhere('performance_status', 1)->orWhere('performance_status', 2)->orWhere('performance_status', 5)->orderBy('performance_status')->getAll('draft');
      ...
      $this->load->view('template', compact('main_view', 'pages', 'performance_layouter'));
	    ...
	    {
	      $performance_layouter = $this->performance->select(['draft.draft_id','draft_title','username','category_name','layout_start_date','layout_deadline','layout_end_date','is_layout','performance_status'])->join3('category','draft','category')->join3('responsibility','draft','draft')->join3('user','responsibility','user')->where('level', 'layouter')->where('performance_status',3)->where('responsibility.user_id !=', null)->orderBy('performance_status')->getAll('draft');
