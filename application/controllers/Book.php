...
class Book extends Operator_Controller {
    ...
    public function index($page = null) {
        ...
        $this->db->group_by('draft.draft_id');
        $tot = $this->book->join('draft')->join3('category', 'draft', 'category')->join3('draft_author','draft','draft')->join3('author','draft_author','author')->join3('work_unit','author','work_unit')->orderBy('status_hak_cipta')->orderBy('published_date')->orderBy('book_title')->getAll();
        //tampilkan author
    ...
    public function search($page = null) {
        ...
        $this->db->group_by('draft.draft_id');
        $tot = $this->book->like('book_code', $keywords)->orLike('draft_title', $keywords)->orLike('book_title', $keywords)->orLike('ISBN', $keywords)->orLike('author_name', $keywords)->join('draft')->join3('category', 'draft', 'category')->join3('draft_author','draft','draft')->join3('author','draft_author','author')->join3('work_unit','author','work_unit')->orderBy('status_hak_cipta')->orderBy('published_date')->orderBy('book_title')->paginate($page)->getAll();
        $total = count($tot);
    ...
    function update_judul(){
        $book_id = $this->input->post('judul_buku_id');
        $judul = $this->input->post('judul_buku_edit');
        $data = array('book_title' => $judul);
        if($this->db->update('book', $data, "book_id='$book_id'")){
            $this->session->set_flashdata('success', 'Data saved');
        }else{
            $this->session->set_flashdata('error', 'Data failed to update');
        }
        redirect('book/view/'.$book_id);
    }
}
