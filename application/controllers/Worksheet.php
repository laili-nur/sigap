    ...
    public function edit($id = null) {
        ...
        if ($this->worksheet->where('worksheet_id', $id)->update($input)) {
            if ($input->worksheet_status == 1) {
                ...
                $judul_notif = $draft->draft_title.' telah disetujui';
                $isi_notif = $draft->draft_title.' telah disetujui dan akan masuk ke proses berikutnya';
            } elseif ($input->worksheet_status == 2) {
                $status = array('draft_status' => 2);
                $judul_notif = $draft->draft_title.' telah ditolak';
                $isi_notif = $draft->draft_title.' ditolak, harap cek kembali';
            } else {
                $status = array('draft_status' => 0);
            }
            $this->worksheet->updateDraftStatus($worksheet->draft_id, $status);

            //select * from draft_author d join author a on d.author_id=a.author_id where draft_id='891'
            if($input->worksheet_status != 0){
                $q_author = $this->db->query("select * from draft_author d join author a on d.author_id=a.author_id where draft_id='".$worksheet->draft_id."'");
