            ...
            <table class="table table-striped">
              ...
              <thead>
                <tr>
                  ...
                  <th scope="col" style="min-width:300px;">Penulis</th>
                  <th scope="col" style="min-width:450px;">Fakultas</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="align-middle">
                    <?php
                    foreach(get_penulis_buku($book->draft_id)->result() as $key => $auth_value){
                        if($key==0){
                            echo '- '.$auth_value->author_name;
                        }else{
                            echo '<br />- '.$auth_value->author_name;
                        }
                    }
                    ?>
                  </td>
                  <td class="align-middle">
                    ...
                    foreach(get_penulis_buku($book->draft_id)->result() as $key => $auth_value){
                        if($key==0){
                            echo '- '.$auth_value->work_unit_name;
                        }else{
                            echo '<br />- '.$auth_value->work_unit_name;
                        }
                    }
                    ?>
                  </td>
...
<?php
function get_penulis_buku($draft_id){
  $CI =& get_instance();
  $query = $CI->db->query("select * from draft_author da join author a on da.author_id=a.author_id join work_unit w on a.work_unit_id=w.work_unit_id where draft_id='$draft_id' order by draft_author_id asc");
  return $query;
}
?>