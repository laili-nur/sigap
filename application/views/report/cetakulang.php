...
<div class="tab-content isi-tab">
    <div class="tab-pane active" id="grafik">
        ...
        <div class="close">+</div>
        <form action="">
          <a href="javascript:embed('<?=base_url('draft/filter?reprint=y&filter=review') ?>')" class="badge badge-primary">Review</a>
          <a href="javascript:embed('<?=base_url('draft/filter?reprint=y&filter=edit') ?>')" class="badge badge-primary">Editorial</a>
          <a href="javascript:embed('<?=base_url('draft/filter?reprint=y&filter=layout') ?>')" class="badge badge-primary">Layout</a>
          <a href="javascript:embed('<?=base_url('draft/filter?reprint=y&filter=proofread') ?>')" class="badge badge-primary">Proofread</a>
          <a href="javascript:embed('<?=base_url('draft/filter?reprint=y&filter=cetak') ?>')" class="badge badge-primary">Cetak</a>
          <a href="javascript:embed('<?=base_url('draft/filter?reprint=y&filter=final') ?>')" class="badge badge-primary">Final</a>
        </form>
        <div style="overflow: hidden; margin: 15px auto;">
        <iframe id="embed" src="<?=base_url('draft/filter?reprint=y&filter=review') ?>" style="border: 0px none; margin-left: -250px; height: 812px; margin-top: -200px; width: 1200px;">
        </iframe>
        </div>
        ...
        $.post("<?php echo base_url();?>Reporting/getSummaryUlang?droptahunsummary="+tahun+"&category="+category,
        function(data){
    ...
    <div class="tab-pane" id="laporan">
      <form id="ffilter">
        ...
        <div class="container mt-2">
          <form id="fexport" method="post" action="<?php echo site_url('reporting/export_cetak_ulang'); ?>" target="_blank">
            <div class="mb-2">
...
<script type="text/javascript">
  $(function(){
    ...
    var table = $('#tbdata').DataTable({
            ...
            "pageLength": 50,
            "ajax": {
                  url: "<?php echo site_url(); ?>reporting/ajax_cetak_ulang/",
                  "type": "POST",
                  "data" : function(d){
                    d.dari= $("#dari").val(),
                    d.sampai= $('#sampai').val()
