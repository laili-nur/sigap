  ...
  <nav aria-label="breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active">
        <a class="text-muted">Performa Layouter Cover</a>
      </li>
    </ol>
  </nav>
...
<ul nav class="nav nav-tabs">
  ...
  <li class="nav-item"><a class="nav-link active" href="<?= base_url('performance/performa_cover') ?>">Performa Layouter Cover</a></li>
...
<br/>
<div class="dropdown">
  <button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown">Performa Layouter Cover
  </button>
  <ul class="dropdown-menu">
    <li class="nav-item"><a class="nav-link" href="<?= base_url('performance/performa_cover') ?>">Performa Cover Process</a></li>
    <li class="nav-item"><a class="nav-link" href="<?= base_url('performance/performa_cover_final') ?>">Performa Cover Final</a></li>
    <li class="nav-item"><a class="nav-link" href="<?= base_url('performance/performa_cover_ontime') ?>">Performa Cover On Time</a></li>
    <li class="nav-item"><a class="nav-link" href="<?= base_url('performance/performa_cover_late') ?>">Performa Cover Late</a></li>
    <li class="nav-item"><a class="nav-link" href="<?= base_url('performance/performa_cover_error') ?>">Performa Cover Error</a></li>
    <li class="nav-item"><a class="nav-link" href="<?= base_url('performance/performa_cover_null') ?>">Performa Cover Kosong</a></li>
  </ul>
</div>
<!-- Reporting buku -->
<!-- /.page-title-bar -->
...
<div align="center">
  <h4>UGM Press</h4>
  <h5>Laporan Performa Staf Layouter Cover</h5>
</div>
<br/>
<style >
table{
...
<div class="container">
  <div class="table-responsive">
    <table id="covertable" class="table table-bordered editortable">
      <thead>
        <tr>
      ...
      <tbody>
        <?php
        if($performance_cover)
        {
          $draft_id = "";
          foreach ($performance_cover as $row)
          {
            if($draft_id != $row->draft_id){
            ?>
            <tr>
              <td><?php echo $row->username; ?></td>
              <td class="align-middle"><strong><a href="<?= base_url('draft/view/' . $row->draft_id . '') ?>"><?= $row->draft_title ?></a></strong></td>
              <td><?php echo $row->category_name; ?></td>
              <td><?php echo konversiTanggal($row->cover_start_date); ?></td>
              <td><?php echo konversiTanggal($row->cover_deadline); ?></td>
              <td><?php echo konversiTanggal($row->cover_end_date); ?></td>
              <td><?php
