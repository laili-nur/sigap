    ...
    <div class="card-body">
      <!-- .progress-list -->
        <ol class="progress-list mb-0 mb-sm-4">
          ...
          <!-- COVER -->
          <li class="
          <?= ($input->is_layout == 'y' and $input->is_cover == 'n' and $input->stts == 99)? 'error':'' ?>
          <?= ($input->is_cover == 'y') ? 'success' : '' ?>
          <?= ($input->is_layout == 'y') ? 'active' : '' ?>
          ">
            <button type="button" data-toggle="tooltip" title="
            <?php if($input->is_cover == 'n' and $input->stts == 99){
              echo 'Ditolak';
            }elseif($input->is_cover == 'y'){
              echo 'Selesai';
            }elseif(konversiTanggal($input->cover_start_date) != '-'){
              echo 'Progress';
            }else{
              echo 'Belum Mulai';
            }?>">
            ...
            <span class="progress-label d-none d-sm-inline-block">Cover</span>
          </li>
          <!-- PROOFREAD -->
          <li class="
          <?= ($input->is_cover == 'y' and $input->is_proofread == 'n' and $input->stts == 99)? 'error':'' ?>
          <?= ($input->is_proofread == 'y') ? 'success' : '' ?>
          <?= ($input->is_cover == 'y') ? 'active' : '' ?>
          ">
          ...
          <?php if($input->is_layout=='y' and ($tot_revisi['editor'] != 0 or $tot_revisi['layouter'] != 0)){
            ...
            }elseif(konversiTanggal($input->proofread_start_date) != '-' && $input->is_layout=='y' && $input->is_cover=='y'){
              echo 'Progress';
            }else{
              echo 'Belum Mulai';
            }?>">
